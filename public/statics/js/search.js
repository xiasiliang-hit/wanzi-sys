...
function getSearch2(string) {
    var htmlStr = "";
    var _data = new Object();
    _data.keyword = string;
    setTimeout(function () {
        $.ajax({
            url: '/service/getSearch',
            type: "POST",
            dataType: "json",
            data: _data,
            cache: false,
            ...
            success: function (data) {
                htmlStr += '<ul>';
                for (var i in data) {
                    htmlStr += '<li class="text-overflow" data-id="' + data[i].areaId + '" data-name="' + data[i].areaName + '">' + data[i].areaIndex + '</li>';
                }
                htmlStr += '</ul>';
                $('.searching-area').html(htmlStr);
            }
            });
    }, 0);
