		...
		if (info) {
			...
			for (var i in data) {
				html += '<div class="order-list"><div class="row order-title"><div class="col-sm-9 col-xs-7"><span><strong>' + data[i].ol_ctime + '</strong></span>\
						...
						<div class="col-sm-6 col-xs-4 text-right"><a href="/site/orderDetail/id/' + data[i].ol_id + '/status/' + config.data.status + '" class="order-detail text-center">查看详情</a></div></div></div>';
			}
			$('#' + config.list_id).html(html);
		}
	}
